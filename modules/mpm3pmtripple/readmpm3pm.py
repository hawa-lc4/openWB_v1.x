...
resp = client.read_input_registers(0x015A,2, slave=idadd)
llkwh = struct.unpack('>f',struct.pack('>HH',*resp.registers))[0]
llkwh = float("%.3f" % llkwh)
f = open('/var/www/html/openWB/ramdisk/llkwh', 'w')
f.write(str(llkwh))
f.close()
resp = client.read_input_registers(0x015C,2, slave=idadd)
llkwh = struct.unpack('>f',struct.pack('>HH',*resp.registers))[0]
llkwh = float("%.3f" % llkwh)
f = open('/var/www/html/openWB/ramdisk/llkwhs1', 'w')
f.write(str(llkwh))
f.close()
resp = client.read_input_registers(0x015E,2, slave=idadd)
llkwh = struct.unpack('>f',struct.pack('>HH',*resp.registers))[0]
llkwh = float("%.3f" % llkwh)
