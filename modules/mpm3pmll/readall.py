...
try:
    ...
    try:
        meter_unit_id = 5
        resp = client.read_input_registers(0x10, 2, slave=meter_unit_id)
    ...
    try:
        meter_unit_id = 105
        resp = client.read_input_registers(0x00, 2, slave=meter_unit_id)
        if type(resp) == ModbusIOException:
            print("no response from id " + str(meter_unit_id))
            raise Exception
    ...
    try:
        meter_unit_id = 201
        resp = client.read_holding_registers(0x5B00, 2, slave=meter_unit_id)
        if type(resp) == ModbusIOException:
            print("no response from id " + str(meter_unit_id))
            raise Exception
...
try:
    if (unit_id < 100):
        resp = client.read_input_registers(0x0002, 4, slave=unit_id)
        ...
        resp = client.read_input_registers(0x12, 2, slave=unit_id)
        lla3 = resp.registers[1]
        lla3 = float(lla3) / 100
        write_to_ramdisk('lla3', lla3)

        resp = client.read_input_registers(0x26, 2, slave=unit_id)
        value1 = resp.registers[0]
        value2 = resp.registers[1]
        all = format(value1, '04x') + format(value2, '04x')
        ...
        resp = client.read_input_registers(0x08, 4, slave=unit_id)
        voltage = resp.registers[1]
        voltage = float(voltage) / 10
        write_to_ramdisk('llv1', voltage)

        resp = client.read_input_registers(0x0A, 4, slave=unit_id)
        voltage = resp.registers[1]
        voltage = float(voltage) / 10
        write_to_ramdisk('llv2', voltage)

        resp = client.read_input_registers(0x0C, 4, slave=unit_id)
        voltage = resp.registers[1]
        voltage = float(voltage) / 10
        write_to_ramdisk('llv3', voltage)
        resp = client.read_input_registers(0x2c, 4, slave=unit_id)
        value1 = resp.registers[0]
        value2 = resp.registers[1]
        all = format(value1, '04x') + format(value2, '04x')
        ...
        if not os.path.isfile("/var/www/html/openWB/ramdisk/lp1Serial"):
            ...
            try:
                resp = client.read_holding_registers(0xFC00, 2, slave=unit_id)
        ...
        # llkwh
        resp = client.read_holding_registers(0x5000, 4, slave=unit_id)
        value1 = resp.registers[0]
        value2 = resp.registers[1]
        ...
        write_to_ramdisk('llkwh', ikwh)
        # Voltage
        resp = client.read_holding_registers(0x5B00, 2, slave=unit_id)
        voltage = resp.registers[1]
        voltage = float(voltage) / 10
        write_to_ramdisk('llv1', voltage)
        resp = client.read_holding_registers(0x5B02, 2, slave=unit_id)
        voltage = resp.registers[1]
        voltage = float(voltage) / 10
        write_to_ramdisk('llv2', voltage)
        resp = client.read_holding_registers(0x5B04, 2, slave=unit_id)
        voltage = resp.registers[1]
        voltage = float(voltage) / 10
        write_to_ramdisk('llv3', voltage)
        # Ampere
        resp = client.read_holding_registers(0x5B0C, 2, slave=unit_id)
        amp = resp.registers[1]
        amp = float(amp) / 100
        write_to_ramdisk('lla1', amp)
        resp = client.read_holding_registers(0x5B0E, 2, slave=unit_id)
        amp = resp.registers[1]
        amp = float(amp) / 100
        write_to_ramdisk('lla2', amp)
        resp = client.read_holding_registers(0x5B10, 2, slave=unit_id)
        amp = resp.registers[1]
        amp = float(amp) / 100
        write_to_ramdisk('lla3', amp)

        # Gesamt watt
        resp = client.read_holding_registers(0x5B14, 2, slave=unit_id)
        value1 = resp.registers[0]
        value2 = resp.registers[1]
        all = format(value1, '04x') + format(value2, '04x')
        final = int(struct.unpack('>i', all.decode('hex'))[0]) / 100
        ...
        write_to_ramdisk('llaktuell', final)
        # LL Hz
        resp = client.read_holding_registers(0x5B2C, 2, slave=unit_id)
