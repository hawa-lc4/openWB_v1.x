...
class SunnyBoyBat:
    ...
    def read(self) -> BatState:
        ...
        imp = self.__tcp_client.read_holding_registers(31393, ModbusDataType.INT_32, slave=unit)
        exp = self.__tcp_client.read_holding_registers(31395, ModbusDataType.INT_32, slave=unit)
        if imp > 5:
            power = imp
        else:
            power = exp * -1

        exported = self.__tcp_client.read_holding_registers(31401, ModbusDataType.UINT_64, slave=3)
        imported = self.__tcp_client.read_holding_registers(31397, ModbusDataType.UINT_64, slave=3)
