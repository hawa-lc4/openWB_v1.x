...
class SmaSunnyBoyInverter:
    ...
    def read(self) -> InverterState:
        if self.component_config.configuration.version == SmaInverterVersion.default:
            ...
            power_total = self.tcp_client.read_holding_registers(30775, ModbusDataType.INT_32, slave=3)
            # Gesamtertrag (Wh) [E-Total]
            energy = self.tcp_client.read_holding_registers(30529, ModbusDataType.UINT_32, slave=3)
            ...
            dc_power = (self.tcp_client.read_holding_registers(30773, ModbusDataType.INT_32, slave=3) +
                        self.tcp_client.read_holding_registers(30961, ModbusDataType.INT_32, slave=3))
        elif self.component_config.configuration.version == SmaInverterVersion.core2:
            # AC Wirkleistung über alle Phasen (W) [Pac]
            power_total = self.tcp_client.read_holding_registers(40084, ModbusDataType.INT_16, slave=1) * 10
            # Gesamtertrag (Wh) [E-Total] SF=2!
            energy = self.tcp_client.read_holding_registers(40094, ModbusDataType.UINT_32, slave=1) * 100
            dc_power = self.tcp_client.read_holding_registers(40101, ModbusDataType.UINT_32, slave=1) * 100
        elif self.component_config.configuration.version == SmaInverterVersion.datamanager:
            # AC Wirkleistung über alle Phasen (W) [Pac]
            power_total = self.tcp_client.read_holding_registers(30775, ModbusDataType.INT_32, slave=2)
            # Total eingespeiste Energie auf allen Außenleitern (Wh) [E-Total]
            energy = self.tcp_client.read_holding_registers(30513, ModbusDataType.UINT_64, slave=2)
