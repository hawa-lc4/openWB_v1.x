    ...
    def update(self):
        with self.__tcp_client:
            voltages = [self.__tcp_client.read_holding_registers(
                reg, ModbusDataType.UINT_32, slave=71) * 0.001 for reg in [62, 102, 142]]
            currents = [self.__tcp_client.read_holding_registers(
                reg, ModbusDataType.UINT_32, slave=71) * 0.001 for reg in [60, 100, 140]]
            power_factors = [self.__tcp_client.read_holding_registers(
                reg, ModbusDataType.INT_32, slave=71) * 0.001 for reg in [64, 104, 144]]
            imported, exported = [val * 0.1 for val in self.__tcp_client.read_holding_registers(
                512, [ModbusDataType.UINT_64]*2, slave=71)]
