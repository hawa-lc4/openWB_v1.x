...
if ("fsp" in status_goe):
    try:
        ...
        if (args.phases == 1 and int(status_goe['fsp']) != 1):
            set_psm_goe = requests.get('http://'+args.address+'/mqtt?payload=psm=1', timeout=5).json()
        ...
        if (args.phases == 3 and int(status_goe['fsp']) != 0):
            if (args.minampere and args.minampere >= 6 and args.minampere <= 16):
                set_amp_goe = requests.get('http://'+args.address+'/mqtt?payload=amp='+str(args.minampere), timeout=5).json()
                if (set_amp_goe['amp'] is True and args.verbose):
                    print("Setzen von MinAmpere erfolgreich!")
            set_psm_goe = requests.get('http://'+args.address+'/mqtt?payload=psm=2', timeout=5).json()
            if (set_psm_goe['psm'] is True and args.verbose):
